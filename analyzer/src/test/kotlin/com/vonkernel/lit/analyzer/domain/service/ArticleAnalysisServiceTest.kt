...
class ArticleAnalysisServiceTest {
    ...
    fun setUp() {
        service = ArticleAnalysisService(
            ...
            keywordAnalyzer, topicAnalyzer, locationAnalyzer,
            geocodingPort, analysisResultRepository
        )
    }
    ...
    private fun setupDefaultAnalyzerMocks(extractedLocations: List<ExtractedLocation> = emptyList()) {
        ...
        coEvery { locationAnalyzer.analyze(testRefinedArticle.title, testRefinedArticle.content) } returns extractedLocations
    ...
    @Test
    @DisplayName("2단계 파이프라인: refine 후 5개 분석기 결과 + geocoding이 AnalysisResult에 올바르게 조합되어 저장된다")
