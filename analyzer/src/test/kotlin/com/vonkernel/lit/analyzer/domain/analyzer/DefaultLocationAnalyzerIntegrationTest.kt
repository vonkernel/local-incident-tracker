...
class DefaultLocationAnalyzerIntegrationTest {
    ...
    fun `재난 기사에서 위치 정보를 추출한다`() = runTest {
        // Given
        val title = "부산 해운대 해수욕장 인근 건물 붕괴 사고"
        val content = "부산광역시 해운대구 중동 해운대 해수욕장 인근의 노후 건물이 붕괴되는 사고가 발생했다. " +
            "사고 현장은 부산 지하철 2호선 해운대역에서 도보 5분 거리에 위치하고 있다. " +
            "부산소방재난본부는 구조대를 긴급 투입하여 매몰자 수색 작업을 진행 중이다."

        // When
        val result = locationAnalyzer.analyze(title, content)
    ...
    @Test
    fun `재난 기사에서 위치 정보를 추출한다2`() = runTest {
        // Given
        val title = "충남도, 한파 쉼터에 난방비 지원…쉼터당 20만원"
        val content = "충남도는 마을회관 및 경로당 등 도내 한파 쉼터에 난방비 10억7천580만원을 지원한다고 28일 밝혔다. " +
