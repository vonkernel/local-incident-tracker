...
class locationsExtractorTest {
    ...
    private lateinit var service: LocationsExtractor
    ...
    @BeforeEach
    fun setUp() {
        service = LocationsExtractor(locationAnalyzer, locationValidator, geocoder)
    }
    ...
    fun analyze_addressType_resolvesViaGeocode() = runTest {
        ...
        coEvery { geocoder.geocodeByAddress("서울 강남구") } returns listOf(testLocation)
    ...
    fun analyze_addressType_emptyResult_returnsUnresolved() = runTest {
        ...
        setupDefaultMocks(extracted)
        coEvery { geocoder.geocodeByAddress("강남역") } returns emptyList()

        // When
        val result = service.process(articleId, title, content)

        // Then
        coVerify(exactly = 1) { geocoder.geocodeByAddress("강남역") }
        coVerify(exactly = 0) { geocoder.geocodeByKeyword(any()) }
    ...
    fun analyze_landmarkType_fallbackToAddress() = runTest {
        ...
        setupDefaultMocks(extracted)
        coEvery { geocoder.geocodeByKeyword("남산타워") } returns emptyList()
        coEvery { geocoder.geocodeByAddress("남산타워") } returns listOf(testLocation)

        // When
        val result = service.process(articleId, title, content)

        // Then
        coVerify(exactly = 1) { geocoder.geocodeByKeyword("남산타워") }
        coVerify(exactly = 1) { geocoder.geocodeByAddress("남산타워") }
        assertThat(result).containsExactly(testLocation)
    }
    ...
    fun analyze_unresolvableType_createsUnknownLocation() = runTest {
        ...
        // Then
        coVerify(exactly = 0) { geocoder.geocodeByAddress(any()) }
        coVerify(exactly = 0) { geocoder.geocodeByKeyword(any()) }
        assertThat(result).hasSize(1)
        assertThat(result[0].coordinate).isNull()
        assertThat(result[0].address.regionType).isEqualTo(RegionType.UNKNOWN)
    ...
    fun analyze_multipleLocations_parallelGeocoding() = runTest {
        ...
        setupDefaultMocks(extracted)
        coEvery { geocoder.geocodeByAddress("서울 강남구") } returns listOf(location1)
        coEvery { geocoder.geocodeByKeyword("남산타워") } returns listOf(location2)
    ...
    fun analyze_validatorFiltersAndNormalizesLocations() = runTest {
        ...
        coEvery { geocoder.geocodeByAddress("충청남도 부여군") } returns listOf(testLocation)

        // When
        val result = service.process(articleId, title, content)

        // Then
        coVerify(exactly = 1) { geocoder.geocodeByAddress("충청남도 부여군") }
        coVerify(exactly = 0) { geocoder.geocodeByAddress("충남 부여군") }
        coVerify(exactly = 0) { geocoder.geocodeByAddress("부여") }
        assertThat(result).containsExactly(testLocation)
    }
}
