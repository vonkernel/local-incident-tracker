...
class DefaultUrgencyAnalyzerIntegrationTest {
    ...
    fun `긴급 재난 기사의 긴급도를 높게 평가한다`() = runTest {
        ...
        val article = Article(
            ...
            title = "경북 포항 규모 5.4 지진 발생…건물 붕괴·부상자 다수",
            content = "오늘 오후 경북 포항시 북구에서 규모 5.4의 지진이 발생했다. " +
                "지진으로 인해 다수의 건물이 붕괴되었으며, 현재까지 부상자 50여 명이 보고되었다. " +
                "여진이 계속되고 있어 주민 대피가 진행 중이다."
        )

        // When
        val result = urgencyAnalyzer.analyze(article)
    ...
    fun `일반 정보성 기사의 긴급도를 낮게 평가한다`() = runTest {
        ...
        val article = Article(
            ...
            writtenAt = Instant.now(),
            modifiedAt = Instant.now(),
            title = "소방청, 올해 화재 예방 캠페인 실시",
            content = "소방청이 올해 가을철 화재 예방 캠페인을 전국적으로 실시한다고 밝혔다. " +
