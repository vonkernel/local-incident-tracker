...
| 유형 | 설명 | 예시 | API 호출 |
|------|------|------|----------|
| `ADDRESS` | 행정구역 단위(시/도, 시/군/구, 읍/면/동, 리, 번지)만으로 구성된 주소. 시설명·건물명·도로명 등 비행정 요소는 제외하여 추출 | "하남시 선동", "경북 포항시 남구 호동", "서울특별시 강남구" | 주소 검색 API |
| `LANDMARK` | 건물명, 시설명, 랜드마크 등 고유명사 장소. 행정구역 접두어는 유지하되, 수식어(인근, 앞, 부근, 일대 등)는 제거 | "코엑스", "부산 해운대 해수욕장", "서울역" | 키워드 장소 검색 API → 주소 검색 API |
...
응답 documents[0].address 매핑 → Location 도메인 모델
    ↓ (응답 없을 경우)
unresolvedLocation(coordinate=null, addressName="하남시 선동")
```

> ADDRESS 유형은 프롬프트에서 행정구역 단위만으로 구성되도록 강제하므로, 키워드 검색 fallback 없이 주소 검색 API 단일 호출로 충분하다.
