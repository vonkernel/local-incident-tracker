...
Local 개발 환경을 위한 인프라 구성 (PostgreSQL, Kafka, Debezium, Kafka UI, OpenSearch)
...
| **OpenSearch** | 9200 | 검색 인덱스 (Read Model) |
| **OpenSearch Dashboards** | 5601 | OpenSearch 관리 웹 인터페이스 |
...
|-------|----------|----------------|
| `lit.analyzer.article-events.dlq` | analyzer (처리 실패 시) | `analyzer-dlq-group` |
| `lit.indexer.analysis-events.dlq` | indexer (처리 실패 시) | `indexer-dlq-group` |
...
│   ├── delete-dlq-topic.sh            # analyzer DLQ 토픽 + consumer group 삭제
